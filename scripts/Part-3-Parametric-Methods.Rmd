---
title: "Titanic: Machine Learning from Disaster, Part III - Parametric Learning Algo"
...
We implement a SVM with different kernels - linear, radial and polynomial
...
2.  We now fit a neural network with 2 hidden layers and 10 neurons per hidden layer.
...
2.  We now fit a neural network with just one hidden layer but greater number of neurons.
...
1.  There is a reasonable increase in the estimated test error over the baseline.
2.  We now fit a neural network with just 2 hidden layers with 15 neurons per hidden layer.
...
Overall, the neural network learning model doesn't seem appropriate for this particular problem.
